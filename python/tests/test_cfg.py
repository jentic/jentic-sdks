...
def test_cfg_from_env__happy_path(monkeypatch):
    ...
    assert cfg.core_api_url == "https://api.jentic.com/api/v1/"
    assert cfg.directory_url == "https://directory-api.qa1.eu-west-1.jenticdev.net/api/v1/"

    monkeypatch.setenv("JENTIC_AGENT_API_KEY", "ak_19814bi2f98jhwg")
    monkeypatch.setenv("JENTIC_ENVIRONMENT", "test")
    cfg = AgentConfig.from_env()
    assert cfg.environment == "test"
    assert cfg.core_api_url == "https://api.test.jentic.com/api/v1/"

    monkeypatch.setenv("JENTIC_AGENT_API_KEY", "ak_19814bi2f98jhwg")
    monkeypatch.setenv("JENTIC_ENVIRONMENT", "qa")
    cfg = AgentConfig.from_env()
    assert cfg.environment == "qa"
    assert cfg.core_api_url == "https://yq6wol1jye.execute-api.eu-west-1.amazonaws.com/api/v1/"
